...
func (nec *necon) ApplyCPUAffinity(pod v1.Pod) error {
	...
	*/
	...
	s1, ok := pod.Annotations["example.com/SLO1"]
	if !ok { return fmt.Errorf("SLO1 annotation 없음") }
	s2, ok := pod.Annotations["example.com/SLO2"]
	if !ok { return fmt.Errorf("SLO2 annotation 없음") }
        coreToExclude1, err := strconv.Atoi(s1)
	if err != nil {
                return fmt.Errorf("SLO1 필드 변환 오류: %v", err)
        }
	coreToExclude2, err := strconv.Atoi(s2)
	if err != nil {
                return fmt.Errorf("SLO2 필드 변환 오류: %v", err)
        }
